...
class CheckTask(Base, TimestampMixin):
    ...
    mockup_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("mockups.id"), nullable=False)
    pen_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("pen_documents.id"), nullable=False)
...
class CheckResult(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    task_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("check_tasks.id"), nullable=False)
